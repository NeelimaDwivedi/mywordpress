...
class Mymetabox {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Mymetabox_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		...
		if ( defined( 'MYMETABOX_VERSION' ) ) {

			$this->version = MYMETABOX_VERSION;
		} else {

			$this->version = '1.0.0';
		}

		$this->plugin_name = 'mymetabox';

		$this->mymetabox_dependencies();
		$this->mymetabox_locale();
		$this->mymetabox_admin_hooks();
		$this->mymetabox_public_hooks();
	 ...
	 *
	 * - Mymetabox_Loader. Orchestrates the hooks of the plugin.
	 * - Mymetabox_i18n. Defines internationalization functionality.
	 * - Mymetabox_Admin. Defines all hooks for the admin area.
	 * - Mymetabox_Public. Defines all hooks for the public side of the site.
	 *
	 ...
	 * @access   private
	 */
	private function mymetabox_dependencies() {

		/**
		 * The class responsible for orchestrating the actions and filters of the
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-mymetabox-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-mymetabox-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-mymetabox-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-mymetabox-public.php';

		$this->loader = new Mymetabox_Loader();
	 ...
	 *
	 * Uses the Mymetabox_I18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	 */
	private function mymetabox_locale() {

		$plugin_i18n = new Mymetabox_I18n();
	 ...
	 * @access   private
	 */
	private function mymetabox_admin_hooks() {

		$m_plugin_admin = new Mymetabox_Admin( $this->m_get_plugin_name(), $this->m_get_version() );
		...
		// Add settings menu for mymetabox.
	 ...
	 * @access   private
	 */
	private function mymetabox_public_hooks() {

		$m_plugin_public = new Mymetabox_Public( $this->m_get_plugin_name(), $this->m_get_version() );
	 ...
	 *
	 * @since     1.0.0
	 * @return    Mymetabox_Loader    Orchestrates the hooks of the plugin.
	 */
	 ...
	 *
	 * @return  Array       An key=>value pair of mymetabox tabs.
	 */
	public function mwb_m_plug_default_tabs() {

		$m_default_tabs = array();

		$m_default_tabs['mymetabox-general'] = array(
			'title'       => esc_html__( 'General Setting', 'mymetabox' ),
			'name'        => 'mymetabox-general',
		);
		$m_default_tabs = apply_filters( 'mwb_m_plugin_standard_admin_settings_tabs', $m_default_tabs );

		$m_default_tabs['mymetabox-system-status'] = array(
			'title'       => esc_html__( 'System Status', 'mymetabox' ),
			'name'        => 'mymetabox-system-status',
		);
	...
	public function mwb_m_plug_load_template( $path, $params = array() ) {
		...
		$m_file_path = MYMETABOX_DIR_PATH . $path;
		...
		if ( file_exists( $m_file_path ) ) {
			...
			$m_notice = sprintf( esc_html__( 'Unable to locate file at location "%s". Some features may not work properly in this plugin. Please contact us!', 'mymetabox' ), $m_file_path );
	...
	public function mwb_m_plug_system_status() {
		...
		// Get PHP version.
		$m_system_status['php_version'] = function_exists( 'phpversion' ) ? phpversion() : __( 'N/A (phpversion function does not exist)', 'mymetabox' );
		...
		// Get the uptime.
		$m_system_status['uptime'] = function_exists( 'exec' ) ? @exec( 'uptime -p' ) : __( 'N/A (make sure exec function is enabled)', 'mymetabox' );

		// Get the server path.
		$m_system_status['server_path'] = defined( 'ABSPATH' ) ? ABSPATH : __( 'N/A (ABSPATH constant not defined)', 'mymetabox' );

		// Get the OS.
		$m_system_status['os'] = function_exists( 'php_uname' ) ? php_uname( 's' ) : __( 'N/A (php_uname function does not exist)', 'mymetabox' );

		// Get WordPress version.
		$m_wordpress_status['wp_version'] = function_exists( 'get_bloginfo' ) ? get_bloginfo( 'version' ) : __( 'N/A (get_bloginfo function does not exist)', 'mymetabox' );

		// Get and count active WordPress plugins.
		$m_wordpress_status['wp_active_plugins'] = function_exists( 'get_option' ) ? count( get_option( 'active_plugins' ) ) : __( 'N/A (get_option function does not exist)', 'mymetabox' );

		// See if this site is multisite or not.
		$m_wordpress_status['wp_multisite'] = function_exists( 'is_multisite' ) && is_multisite() ? __( 'Yes', 'mymetabox' ) : __( 'No', 'mymetabox' );

		// See if WP Debug is enabled.
		$m_wordpress_status['wp_debug_enabled'] = defined( 'WP_DEBUG' ) ? __( 'Yes', 'mymetabox' ) : __( 'No', 'mymetabox' );

		// See if WP Cache is enabled.
		$m_wordpress_status['wp_cache_enabled'] = defined( 'WP_CACHE' ) ? __( 'Yes', 'mymetabox' ) : __( 'No', 'mymetabox' );

		// Get the total number of WordPress users on the site.
		$m_wordpress_status['wp_users'] = function_exists( 'count_users' ) ? count_users() : __( 'N/A (count_users function does not exist)', 'mymetabox' );

		// Get the number of published WordPress posts.
		$m_wordpress_status['wp_posts'] = wp_count_posts()->publish >= 1 ? wp_count_posts()->publish : __( '0', 'mymetabox' );

		// Get PHP memory limit.
		$m_system_status['php_memory_limit'] = function_exists( 'ini_get' ) ? (int) ini_get( 'memory_limit' ) : __( 'N/A (ini_get function does not exist)', 'mymetabox' );

		// Get the PHP error log path.
		$m_system_status['php_error_log_path'] = ! ini_get( 'error_log' ) ? __( 'N/A', 'mymetabox' ) : ini_get( 'error_log' );

		// Get PHP max upload size.
		$m_system_status['php_max_upload'] = function_exists( 'ini_get' ) ? (int) ini_get( 'upload_max_filesize' ) : __( 'N/A (ini_get function does not exist)', 'mymetabox' );

		// Get PHP max post size.
		$m_system_status['php_max_post'] = function_exists( 'ini_get' ) ? (int) ini_get( 'post_max_size' ) : __( 'N/A (ini_get function does not exist)', 'mymetabox' );
		...
		$m_system_status['server_hostname'] = function_exists( 'gethostname' ) ? gethostname() : __( 'N/A (gethostname function does not exist)', 'mymetabox' );

		// Show the number of processes currently running on the server.
		$m_system_status['processes'] = function_exists( 'exec' ) ? @exec( 'ps aux | wc -l' ) : __( 'N/A (make sure exec is enabled)', 'mymetabox' );
		...
		if ( stristr( PHP_OS, 'win' ) ) {
			...
			$m_system_status['windows_cpu_usage'] = function_exists( 'exec' ) ? @exec( 'wmic cpu get loadpercentage /all' ) : __( 'N/A (make sure exec is enabled)', 'mymetabox' );
		}

		// Get the memory limit.
		$m_system_status['memory_limit'] = function_exists( 'ini_get' ) ? (int) ini_get( 'memory_limit' ) : __( 'N/A (ini_get function does not exist)', 'mymetabox' );

		// Get the PHP maximum execution time.
		$m_system_status['php_max_execution_time'] = function_exists( 'ini_get' ) ? ini_get( 'max_execution_time' ) : __( 'N/A (ini_get function does not exist)', 'mymetabox' );

		// Get outgoing IP address.
		$m_system_status['outgoing_ip'] = function_exists( 'file_get_contents' ) ? file_get_contents( 'http://ipecho.net/plain' ) : __( 'N/A (file_get_contents function does not exist)', 'mymetabox' );
