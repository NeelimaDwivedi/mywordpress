...
global $m_mwb_m_obj;
$m_active_tab   = isset( $_GET['m_tab'] ) ? sanitize_key( $_GET['m_tab'] ) : 'metaglobal-general';
		...
		<div class="mwb-m-go-pro-banner">
			<div class="mwb-m-inner-container">
				<div class="mwb-m-name-wrapper">
					<p><?php esc_html_e( 'metaglobal', 'metaglobal' ); ?></p></div>
						...
						<ul>
							...
							<?php if ( isset( $m_plugin_pro_link ) && '' != $m_plugin_pro_link ) { ?>
								<li class="mwb-m-main-menu-button">
									<a id="mwb-m-go-pro-link" href="<?php echo esc_url( $m_plugin_pro_link ); ?>" class="" title="" target="_blank"><?php esc_html_e( 'GO PRO NOW', 'metaglobal' ); ?></a>
								</li>
							<?php } else { ?>
								<li class="mwb-m-main-menu-button">
									<a id="mwb-m-go-pro-link" href="#" class="" title=""><?php esc_html_e( 'GO PRO NOW', 'metaglobal' ); ?></a>
								</li>
							<?php } ?>
							...
							<?php if ( isset( $m_plugin_pro ) && 'yes' == $m_plugin_pro ) { ?>
								<li>
									<a id="mwb-m-skype-link" href="<?php echo esc_url( 'https://join.skype.com/invite/IKVeNkLHebpC' ); ?>" target="_blank">
										<img src="<?php echo esc_url( METAGLOBAL_DIR_URL . 'admin/images/skype_logo.png' ); ?>" style="height: 15px;width: 15px;" ><?php esc_html_e( 'Chat Now', 'metaglobal' ); ?>
									</a>
								</li>
							<?php } ?>
						</ul>
						...
						if ( is_array( $m_default_tabs ) && ! empty( $m_default_tabs ) ) {
							...
							foreach ( $m_default_tabs as $m_tab_key => $m_default_tabs ) {
								...
								<div class="mwb-m-tabs">
									<a class="<?php echo esc_attr( $m_tab_classes ); ?>" id="<?php echo esc_attr( $m_tab_key ); ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=metaglobal_menu' ) . '&m_tab=' . esc_attr( $m_tab_key ) ); ?>"><?php echo esc_html( $m_default_tabs['title'] ); ?></a>
								</div>
