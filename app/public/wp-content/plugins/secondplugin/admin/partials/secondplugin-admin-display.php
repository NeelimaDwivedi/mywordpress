 ...
 * @subpackage Secondplugin/admin/partials
 */
...
global $s_mwb_s_obj;
$s_active_tab   = isset( $_GET['s_tab'] ) ? sanitize_key( $_GET['s_tab'] ) : 'secondplugin-general';
		...
		<div class="mwb-s-go-pro-banner">
			<div class="mwb-s-inner-container">
				<div class="mwb-s-name-wrapper">
					<p><?php esc_html_e( 'secondplugin', 'secondplugin' ); ?></p></div>
						...
						<ul>
							...
							<?php if ( isset( $s_plugin_pro_link ) && '' != $s_plugin_pro_link ) { ?>
								<li class="mwb-s-main-menu-button">
									<a id="mwb-s-go-pro-link" href="<?php echo esc_url( $s_plugin_pro_link ); ?>" class="" title="" target="_blank"><?php esc_html_e( 'GO PRO NOW', 'secondplugin' ); ?></a>
								</li>
							<?php } else { ?>
								<li class="mwb-s-main-menu-button">
									<a id="mwb-s-go-pro-link" href="#" class="" title=""><?php esc_html_e( 'GO PRO NOW', 'secondplugin' ); ?></a>
								</li>
							<?php } ?>
							...
							<?php if ( isset( $s_plugin_pro ) && 'yes' == $s_plugin_pro ) { ?>
								<li>
									<a id="mwb-s-skype-link" href="<?php echo esc_url( 'https://join.skype.com/invite/IKVeNkLHebpC' ); ?>" target="_blank">
										<img src="<?php echo esc_url( SECONDPLUGIN_DIR_URL . 'admin/images/skype_logo.png' ); ?>" style="height: 15px;width: 15px;" ><?php esc_html_e( 'Chat Now', 'secondplugin' ); ?>
									</a>
								</li>
							<?php } ?>
						</ul>
						...
						if ( is_array( $s_default_tabs ) && ! empty( $s_default_tabs ) ) {
							...
							foreach ( $s_default_tabs as $s_tab_key => $s_default_tabs ) {
								...
								<div class="mwb-s-tabs">
									<a class="<?php echo esc_attr( $s_tab_classes ); ?>" id="<?php echo esc_attr( $s_tab_key ); ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=secondplugin_menu' ) . '&s_tab=' . esc_attr( $s_tab_key ) ); ?>"><?php echo esc_html( $s_default_tabs['title'] ); ?></a>
								</div>
