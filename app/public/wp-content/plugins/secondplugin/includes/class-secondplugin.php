...
class Secondplugin {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Secondplugin_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		...
		if ( defined( 'SECONDPLUGIN_VERSION' ) ) {

			$this->version = SECONDPLUGIN_VERSION;
		} else {

			$this->version = '1.0.0';
		}

		$this->plugin_name = 'secondplugin';

		$this->secondplugin_dependencies();
		$this->secondplugin_locale();
		$this->secondplugin_admin_hooks();
		$this->secondplugin_public_hooks();
	 ...
	 *
	 * - Secondplugin_Loader. Orchestrates the hooks of the plugin.
	 * - Secondplugin_i18n. Defines internationalization functionality.
	 * - Secondplugin_Admin. Defines all hooks for the admin area.
	 * - Secondplugin_Public. Defines all hooks for the public side of the site.
	 *
	 ...
	 * @access   private
	 */
	private function secondplugin_dependencies() {

		/**
		 * The class responsible for orchestrating the actions and filters of the
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-secondplugin-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-secondplugin-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-secondplugin-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-secondplugin-public.php';

		$this->loader = new Secondplugin_Loader();
	 ...
	 *
	 * Uses the Secondplugin_I18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	 */
	private function secondplugin_locale() {

		$plugin_i18n = new Secondplugin_I18n();
	 ...
	 * @access   private
	 */
	private function secondplugin_admin_hooks() {

		$s_plugin_admin = new Secondplugin_Admin( $this->s_get_plugin_name(), $this->s_get_version() );
		...
		// Add settings menu for secondplugin.
	 ...
	 * @access   private
	 */
	private function secondplugin_public_hooks() {

		$s_plugin_public = new Secondplugin_Public( $this->s_get_plugin_name(), $this->s_get_version() );
	 ...
	 *
	 * @since     1.0.0
	 * @return    Secondplugin_Loader    Orchestrates the hooks of the plugin.
	 */
	 ...
	 *
	 * @return  Array       An key=>value pair of secondplugin tabs.
	 */
	public function mwb_s_plug_default_tabs() {

		$s_default_tabs = array();

		$s_default_tabs['secondplugin-general'] = array(
			'title'       => esc_html__( 'General Setting', 'secondplugin' ),
			'name'        => 'secondplugin-general',
		);
		$s_default_tabs = apply_filters( 'mwb_s_plugin_standard_admin_settings_tabs', $s_default_tabs );

		$s_default_tabs['secondplugin-system-status'] = array(
			'title'       => esc_html__( 'System Status', 'secondplugin' ),
			'name'        => 'secondplugin-system-status',
		);
	...
	public function mwb_s_plug_load_template( $path, $params = array() ) {
		...
		$s_file_path = SECONDPLUGIN_DIR_PATH . $path;
		...
		if ( file_exists( $s_file_path ) ) {
			...
			$s_notice = sprintf( esc_html__( 'Unable to locate file at location "%s". Some features may not work properly in this plugin. Please contact us!', 'secondplugin' ), $s_file_path );
	...
	public function mwb_s_plug_system_status() {
		...
		// Get PHP version.
		$s_system_status['php_version'] = function_exists( 'phpversion' ) ? phpversion() : __( 'N/A (phpversion function does not exist)', 'secondplugin' );
		...
		// Get the uptime.
		$s_system_status['uptime'] = function_exists( 'exec' ) ? @exec( 'uptime -p' ) : __( 'N/A (make sure exec function is enabled)', 'secondplugin' );

		// Get the server path.
		$s_system_status['server_path'] = defined( 'ABSPATH' ) ? ABSPATH : __( 'N/A (ABSPATH constant not defined)', 'secondplugin' );

		// Get the OS.
		$s_system_status['os'] = function_exists( 'php_uname' ) ? php_uname( 's' ) : __( 'N/A (php_uname function does not exist)', 'secondplugin' );

		// Get WordPress version.
		$s_wordpress_status['wp_version'] = function_exists( 'get_bloginfo' ) ? get_bloginfo( 'version' ) : __( 'N/A (get_bloginfo function does not exist)', 'secondplugin' );

		// Get and count active WordPress plugins.
		$s_wordpress_status['wp_active_plugins'] = function_exists( 'get_option' ) ? count( get_option( 'active_plugins' ) ) : __( 'N/A (get_option function does not exist)', 'secondplugin' );

		// See if this site is multisite or not.
		$s_wordpress_status['wp_multisite'] = function_exists( 'is_multisite' ) && is_multisite() ? __( 'Yes', 'secondplugin' ) : __( 'No', 'secondplugin' );

		// See if WP Debug is enabled.
		$s_wordpress_status['wp_debug_enabled'] = defined( 'WP_DEBUG' ) ? __( 'Yes', 'secondplugin' ) : __( 'No', 'secondplugin' );

		// See if WP Cache is enabled.
		$s_wordpress_status['wp_cache_enabled'] = defined( 'WP_CACHE' ) ? __( 'Yes', 'secondplugin' ) : __( 'No', 'secondplugin' );

		// Get the total number of WordPress users on the site.
		$s_wordpress_status['wp_users'] = function_exists( 'count_users' ) ? count_users() : __( 'N/A (count_users function does not exist)', 'secondplugin' );

		// Get the number of published WordPress posts.
		$s_wordpress_status['wp_posts'] = wp_count_posts()->publish >= 1 ? wp_count_posts()->publish : __( '0', 'secondplugin' );

		// Get PHP memory limit.
		$s_system_status['php_memory_limit'] = function_exists( 'ini_get' ) ? (int) ini_get( 'memory_limit' ) : __( 'N/A (ini_get function does not exist)', 'secondplugin' );

		// Get the PHP error log path.
		$s_system_status['php_error_log_path'] = ! ini_get( 'error_log' ) ? __( 'N/A', 'secondplugin' ) : ini_get( 'error_log' );

		// Get PHP max upload size.
		$s_system_status['php_max_upload'] = function_exists( 'ini_get' ) ? (int) ini_get( 'upload_max_filesize' ) : __( 'N/A (ini_get function does not exist)', 'secondplugin' );

		// Get PHP max post size.
		$s_system_status['php_max_post'] = function_exists( 'ini_get' ) ? (int) ini_get( 'post_max_size' ) : __( 'N/A (ini_get function does not exist)', 'secondplugin' );
		...
		$s_system_status['server_hostname'] = function_exists( 'gethostname' ) ? gethostname() : __( 'N/A (gethostname function does not exist)', 'secondplugin' );

		// Show the number of processes currently running on the server.
		$s_system_status['processes'] = function_exists( 'exec' ) ? @exec( 'ps aux | wc -l' ) : __( 'N/A (make sure exec is enabled)', 'secondplugin' );
		...
		if ( stristr( PHP_OS, 'win' ) ) {
			...
			$s_system_status['windows_cpu_usage'] = function_exists( 'exec' ) ? @exec( 'wmic cpu get loadpercentage /all' ) : __( 'N/A (make sure exec is enabled)', 'secondplugin' );
		}

		// Get the memory limit.
		$s_system_status['memory_limit'] = function_exists( 'ini_get' ) ? (int) ini_get( 'memory_limit' ) : __( 'N/A (ini_get function does not exist)', 'secondplugin' );

		// Get the PHP maximum execution time.
		$s_system_status['php_max_execution_time'] = function_exists( 'ini_get' ) ? ini_get( 'max_execution_time' ) : __( 'N/A (ini_get function does not exist)', 'secondplugin' );

		// Get outgoing IP address.
		$s_system_status['outgoing_ip'] = function_exists( 'file_get_contents' ) ? file_get_contents( 'http://ipecho.net/plain' ) : __( 'N/A (file_get_contents function does not exist)', 'secondplugin' );
